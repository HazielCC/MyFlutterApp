...
class ThemeProvider extends ChangeNotifier {
  ...
  void fontControl() {
    if (hiveLocalStorage.fontStyleBox.isEmpty) {
      hiveLocalStorage.fontStyleBox
          .add(ThemeModel(fonttStyle: FonttStyle.opensans, lightMode: true));
    }
  }

  TextTheme changeFont() {
    fontControl();
    if (hiveLocalStorage.fontStyleBox.getAt(0)?.fonttStyle ==
        FonttStyle.opensans) {
      choosedFont = GoogleFonts.openSansTextTheme();
      return GoogleFonts.openSansTextTheme()
          .copyWith(headline6: themeControl(21, FontWeight.normal));
    } else if (hiveLocalStorage.fontStyleBox.getAt(0)!.fonttStyle ==
        FonttStyle.lato) {
  ...
  TextStyle themeControl(double fontsize, FontWeight fontWeight) {
    if (hiveLocalStorage.fontStyleBox.getAt(0)?.fonttStyle ==
      ...
      return GoogleFonts.openSans(
          ...
          fontSize: fontsize,
          color: hiveLocalStorage.fontStyleBox.getAt(0)!.lightMode == false
              ? Colors.white
              : Colors.black);
    } else if (hiveLocalStorage.fontStyleBox.getAt(0)!.fonttStyle ==
        FonttStyle.lato) {
  ...
  void addFont(FonttStyle fonttStyle) {
    hiveLocalStorage.addThemeModel(
        fontStyleModel: ThemeModel(
            fonttStyle: fonttStyle,
            lightMode: hiveLocalStorage.fontStyleBox.getAt(0)!.lightMode));
    notifyListeners();
  }
  ...
  void switchState(bool state) {
    ...
    hiveLocalStorage.putThemeModel(
        fontStyleModel: ThemeModel(
            fonttStyle: hiveLocalStorage.fontStyleBox.getAt(0)!.fonttStyle,
            lightMode: state));
    notifyListeners();
  }
}
